...
export const returns: ReturnRecord[] = [
  {
    ...
    condition: "good" as const, totalAmount: 0, status: "approved", reason: "اقتراب صلاحية المنتج",
  },
  {
    id: "rt-002", number: "RET-2026-0114", invoiceId: "so-003", customerId: "c-024", repId: "u-rp-02", date: "2026-08-13",
    items: [returnLine("p-001", 6)],
    condition: "damaged" as const, totalAmount: 0, status: "approved", reason: "تلف أثناء النقل — عبوات منبعجة",
  },
  {
    id: "rt-003", number: "RET-2026-0113", invoiceId: "so-010", customerId: "c-012", repId: "u-rp-01", date: "2026-08-13",
    items: [returnLine("p-007", 12)],
    condition: "good" as const, totalAmount: 0, status: "pending", reason: "طلب العميل — فائض من الطلب السابق",
  },
  {
    ...
    condition: "good" as const, totalAmount: 0, status: "approved", reason: "استبدال بمنتجات أحدث",
  },
  {
    id: "rt-005", number: "RET-2026-0111", invoiceId: "so-021", customerId: "c-002", repId: "u-rp-01", date: "2026-08-11",
    items: [returnLine("p-019", 3)],
    condition: "damaged" as const, totalAmount: 0, status: "approved", reason: "عبوات مثقوبة",
  },
  {
    id: "rt-006", number: "RET-2026-0110", invoiceId: "so-013", customerId: "c-017", repId: "u-rp-05", date: "2026-08-10",
    items: [returnLine("p-015", 6)],
    condition: "damaged" as const, totalAmount: 0, status: "rejected", reason: "انتهاء الصلاحية قبل البيع — خارج سياسة المرتجعات",
  },
  {
    id: "rt-007", number: "RET-2026-0109", invoiceId: "so-012", customerId: "c-019", repId: "u-rp-05", date: "2026-08-09",
    items: [returnLine("p-009", 2)],
    condition: "good" as const, totalAmount: 0, status: "approved", reason: "تأخر تسليم — إلغاء جزئي من العميل",
  },
  {
    id: "rt-008", number: "RET-2026-0108", invoiceId: "so-023", customerId: "c-007", repId: "u-rp-04", date: "2026-08-08",
    items: [returnLine("p-012", 2)],
    condition: "good" as const, totalAmount: 0, status: "approved", reason: "خطأ في الكمية المرسلة",
  },
].map((r) => ({ ...r, totalAmount: r.items.reduce((s, i) => s + i.lineTotal, 0) }));
